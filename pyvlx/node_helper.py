...
def convert_frame_to_node(pyvlx, frame):
    ...
    if frame.node_type == NodeTypeWithSubtype.ROLLER_SHUTTER or \
        ...
        return RollerShutter(pyvlx=pyvlx, node_id=frame.node_id, name=frame.name, position=frame.current_position)
    if frame.node_type == NodeTypeWithSubtype.EXTERIOR_VENETIAN_BLIND:
        return Blind(pyvlx=pyvlx, node_id=frame.node_id, name=frame.name, position=frame.current_position, orientation=frame.current_position_fp3)
