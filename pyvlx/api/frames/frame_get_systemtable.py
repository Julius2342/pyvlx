...
class FrameGetSystemTableNotification(FrameBase):
    ...
    def get_payload(self) -> bytes:
        ...
        for number, name in self.actutators:
            ret += self.idx
            ret += self.address
            ret += self.type
            ret += (self.turnAroundTime + self.rf * 16 + self.io * 32 + self.powerSaveMode * 64)
            ret += self.manufactor
            ret += self.backbone
    ...
    def from_payload(self, payload: bytes) -> None:
        ...
        for i in range(number_of_objects):
            self.actutators.append(Actutator(
                ...
                PowerMode( payload[(i * 11 + 7)] >> 6 & 3 ), #bit 0-1 powerSaveMode
