...
from pyvlx.api.frames import ( FrameGetSystemTableRequest, FrameGetSystemTableConfirmation, FrameGetSystemTableNotification )
from pyvlx.const import ( NodeTypeWithSubtype, PowerMode, TurnAround, Manufactor )
...
class TestFrameGetSystemTableRequest(unittest.TestCase):
  ...
  def test_notify_one_from_raw(self):
    ...
    self.assertEqual(frame.actutators[0].type, NodeTypeWithSubtype.VENTILATION_POINT_AIR_INLET)
    self.assertEqual(frame.actutators[0].powerSaveMode, PowerMode.LOW_POWER_MODE)
    self.assertEqual(frame.actutators[0].io,True)
    self.assertEqual(frame.actutators[0].rf,True)
    self.assertEqual(frame.actutators[0].turnAroundTime, TurnAround.WITHIN_20ms)
  ...
  def test_notify_one_str(self):
    """Test parse FrameGetSystemTableNotification from raw."""
    frame = frame_from_raw(self.ONE_NOTIFY)
    self.assertEqual(str(frame),'<FrameGetSystemTableNotification objects="1" remaining_objects="113"><Actutator index="1" address="18.52.86" type="VENTILATION_POINT_AIR_INLET" powerSaveMode="LOW_POWER_MODE" io="true" rf="true" turnAroundTime="WITHIN_20ms" manufactor="OVERKIZ" backbone="101.67.33"/></FrameGetSystemTableNotification>')
